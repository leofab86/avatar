...
def custom_data_optimization(db_profile_id, teacher_levels, class_levels, student_levels, timer):
    db_profile = DatabaseProfile.objects.get(db_profile_id=db_profile_id)
    with timer['custom_optimization'].run('db_profile_evaluate_queryset'):
    ...
    if teacher_levels > 0:
        ...
        with timer['custom_optimization'].run('teachers_query_and_serialize'):
    ...
    if (class_levels > 0 or student_levels == 3) and not include_classes:
        ...
        with timer['custom_optimization'].run('classes_query_and_serialize'):
    ...
    if student_levels > 0 and student_levels != 3 and not include_students:
        ...
        with timer['custom_optimization'].run('students_query_and_serializer'):
    ...
    with timer['custom_optimization'].run('custom_optimization_iterations'):
